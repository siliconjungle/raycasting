...
export const subscribeOT = async (url) => {
  ...
  // let doc = JSON.parse(decoder.decode(first.value.value))
  let doc = decoder.decode(first.value.value)
  let serverVersion = first.value.headers['version']
  // console.log('_SERVER_VERSION_', serverVersion)
  ...
  const sendInflight = async () => {
    ...
    console.log('_INFLIGHT_OP_OP_', inflightOp.op)
    console.log('_INFLIGHT_OP_OP_TYPE_', typeof(inflightOp.op))
    ...
    console.log('_RES_TEXT_', await res.text())
